...
from Tran_clip import tran_clip_d9 as model_vit
...
import make_raven_d9_clip_data as make_data # no wrong answers
...
txt_data = torch.from_numpy(train_set.txt_data[None,:])
...
while epoch < num_epoch:
    ...
    with tqdm(total=len(train_loader)  + len(val_loader)) as pbar:
        ...
        for x_train, label, _ in train_loader:
            ...
            y_train = (txt_data).long().to(device)
            ...
            out_train = model(x_train, y_train.expand(x_train.shape[0], -1, -1)) #输出

            

            loss, choose_right = (model.module.loss_function(*out_train, target = label) if isinstance(model, nn.DataParallel) 
                    else model.loss_function(*out_train, target = label))
        ...
        with torch.no_grad():
            ...
            for index, (x_test, label, _)in enumerate(val_loader):
                ...
                y_test = (txt_data).long().to(device)
                label = label.long().to(device)

                x_test = x_test/255.
                
                out_test = model(x_test, y_test.expand(x_test.shape[0], -1, -1))

                _, choose_right = (model.module.loss_function(*out_test, target = label) if isinstance(model, nn.DataParallel) 
                        else model.loss_function(*out_test, target = label))
    ...
    print("epoch:{}\t loss_train:{:.4f}\t accuracy_train:{:.4f}\t  accuracy_val:{:.4f}\t  learning_rate:{:.8f}".format(epoch, loss_train,   accuracy[1], accuracy_val[1],  lr_decay.get_lr()[0]))

    with open("test_on_"+text_name+".txt", "a") as f:  # 打开文件
        
        f.write("epoch:{}\t loss_train:{:.4f}\t accuracy_train:{:.4f}\t  accuracy_val:{:.4f}\t  learning_rate:{:.8f}".format(epoch, loss_train,   accuracy[1], accuracy_val[1],  lr_decay.get_lr()[0]))
    epoch += 1
