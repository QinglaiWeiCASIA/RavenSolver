...
class Raven_Data(Data.Dataset):
    def __init__(self,
        ...
        assert len(self.file_names_npz) != 0 , 'empty dataset Error' 
    ...
    def shuffle_set(self):
        ...
        self.work_npz = self.file_names_npz[:200000]
...
def raven_loader(batch_size,  train = True, val = False, num_workers = 0):
    dataset = Raven_Data(train = train, val = val)
    return DataLoader(dataset, batch_size = batch_size, sampler = None, collate_fn = my_collate, shuffle = True, num_workers = num_workers), len(dataset)



def make_loader(dataset, batch_size,  num_workers = 0):
