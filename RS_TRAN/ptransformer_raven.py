...
class reasoning(nn.Module):
    def __init__(self, args):
        ...
        self.name = 'RS-Transformer_on_raven'
        ...
        self.g_function = nn.Sequential(
            ...
            Rearrange('(b s w) d -> b s w d', s = 10, w = self.w)
        ...
        self.high_law = nn.Sequential( #b ,8, 6, 16, 3, d
                                        Rearrange('b s v w n d-> (b s v w) n d'),
                                        Clstransformer(words = 3, dim = self.low_dim, depth = num_depth, heads = num_head, dim_head = int(self.low_dim/num_head), mlp_dim = self.low_dim, dropout = 0.1),
                                        Rearrange('(b s v w) n d-> b s v w n d', s = 8, v = 1, w = self.w))
        ...
        self.tajador = nn.Sequential(Rearrange('b s v w n d -> (b s v w n) d'),
                            Bottleneck_judge(self.low_dim, self.low_dim),
                            Rearrange('(b s v w n) d -> b s (v w n d)', s = 8, v = 1, w = self.w, n = 3),
                            Mean(dim = 2))
    ...
    def combine(self, r_law):
        ...
        assert r_law.shape[1] == 10 and r_law.shape[2] == self.w and r_law.shape[3] == self.low_dim
            
        for i in range(8):
            r_combine_0[i] = torch.stack((r_law[:,0], r_law[:,1], r_law[:,2+i]), dim = 2)#b, 16 ,3 ,d, 
           
            
        r_combine_0 = torch.stack(r_combine_0, dim = 1)#b ,8, 16, 3, d
    
        return r_combine_0.unsqueeze(2)
    ...
    def forward(self, x):
        ...
        cat_row = self.cat(x)# b, 10,16 ,3*d
        
        r_law = self.g_function(cat_row)# b, 10, 16,d

        rc_law = self.combine(r_law)#b, 8,  16, 3,d

        high = self.high_law(rc_law)#b, 8,16, 6,d

        return self.tajador(high).reshape(b,8), torch.empty(0).cuda()
